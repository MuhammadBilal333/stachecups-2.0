...
// Initialize collection from URL on page load
onMounted(async () => {
  console.log('🎨 Customizer: Initializing collection system...')
  ...
  if (collectionParam) {
    console.log(`🎨 Customizer: Setting collection from URL: ${collectionParam}`)
    await collectionStore.setCollection(collectionParam, true) // Skip warning on initial load
  } else {
    console.log('🎨 Customizer: Using default collection (general)')
    await collectionStore.setCollection('general', true)
  }

  console.log(`✅ Customizer: Collection active - ${collectionStore.activeCollection}`)
  console.log(`📊 Customizer: Allowed asset tags - ${collectionStore.allowedAssetTags.join(', ')}`)
  console.log(`🎨 Customizer: Can use CMYK - ${collectionStore.canUseCMYK}`)
  console.log(`📤 Customizer: Can upload - ${collectionStore.canUpload}`)
...
watch(() => route.query.collection, async (newCollection) => {
  if (newCollection && typeof newCollection === 'string' && newCollection !== collectionStore.activeCollection) {
    console.log(`🔄 Customizer: Collection changed via URL: ${newCollection}`)
