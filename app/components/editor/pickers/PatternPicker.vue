...
const filteredPatterns = computed(() => {
  console.log(`🔍 Filtering patterns: total=${patterns.value.length}, category=${selectedCategory.value}, search=${searchQuery.value}`)
...
onMounted(() => {
  console.log('🎨 PatternPicker: Initializing patterns...')

  try {
    const initializedPatterns = initializePatterns()
    console.log(`✅ PatternPicker: Generated ${initializedPatterns.length} patterns`)

    // Log first pattern for debugging
    if (initializedPatterns.length > 0) {
      console.log('📊 First pattern:', {
        id: initializedPatterns[0].id,
        name: initializedPatterns[0].name,
        hasPreview: !!initializedPatterns[0].preview,
        previewLength: initializedPatterns[0].preview?.length
      })
    }

    patterns.value = initializedPatterns
  } catch (error) {
    console.error('❌ PatternPicker: Failed to initialize patterns:', error)
    patterns.value = []
  }
  ...
  console.log(`🎨 PatternPicker: Ready with ${patterns.value.length} patterns`)
})
</script>
