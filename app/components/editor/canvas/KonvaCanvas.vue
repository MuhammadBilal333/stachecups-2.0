...
// Filter visible elements and sort by zIndex
...
// Combine and sort all visible elements by zIndex for proper rendering order
...
const shouldShowLeftLoop = (element) => {
  // Only show left loop when element is close to right edge (within 20% of canvas width)
  const edgeThreshold = props.width * 0.8;
  return element.position.x > edgeThreshold;
};

const shouldShowRightLoop = (element) => {
  // Only show right loop when element is close to left edge (within 20% of canvas width)
