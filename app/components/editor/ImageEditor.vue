...
const addToCart = async () => {
  ...
  try {
    ...
    console.log('✅ Received signed URL:', response)
    ...
    const finalImageUrl = response.split('?')[0]
    console.log('📤 Sending data to parent window:', {
      imageUrl: finalImageUrl,
      hasDesignData: !!result.cartItem?.designData,
      hasMetadata: !!result.cartItem?.metadata
    })
    ...
    console.log('✅ Data sent to parent window successfully!')
    ...
    console.log('🏁 S3 upload process finished')
    $q.loading.hide()
  }
}
  ...
  if (loadedFromUrl) {
    ...
    if (savedDesign) {
      ...
      if (currentCollection === savedCollection) {
        ...
        // Different collection - clear the old autosave
        console.log(`🗑️ Clearing autosave from different collection (saved: ${savedCollection}, current: ${currentCollection})`)
        designState.clear()
      }
    }
  }
