...
export const useCollectionStore = defineStore('collection', {
  ...
  getters: {
     ...
     */
    branding() {
      return this.currentCollection?.branding || null
    },
  ...
  actions: {
    ...
    async loadCollections() {
      ...
      try {
        ...
        console.log('✅ Collections loaded:', this.collections.map(c => c.id))
      } catch (error) {
        console.error('❌ Error loading collections:', error)
        // Fallback to general if loading fails
    ...
    async setCollection(collectionId: string, skipWarning = false): Promise<boolean> {
      ...
      if (!targetCollection.active) {
        console.error(`Collection "${collectionId}" is not active`)
        return false
      }

      // If already on this collection, do nothing
      if (this.activeCollection === collectionId) {
        return true
      }

      // If not skipping warning and not initial load, show warning
      if (!skipWarning && this.activeCollection) {
        // Warning will be shown by component
        // Component should call confirmSwitchCollection()
        return false
      }

      this.activeCollection = collectionId
      this.tosAccepted = false // Reset TOS on collection switch

      console.log(`🎨 Switched to collection: ${collectionId}`)
      return true
    },

    /**
     * Confirm collection switch (after user confirms warning)
     */
    confirmSwitchCollection(collectionId: string) {
      this.activeCollection = collectionId
      this.tosAccepted = false

      console.log(`✅ Collection switch confirmed: ${collectionId}`)
    },
